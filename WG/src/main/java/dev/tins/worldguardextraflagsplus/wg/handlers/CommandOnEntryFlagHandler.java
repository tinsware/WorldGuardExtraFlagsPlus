...
public class CommandOnEntryFlagHandler extends Handler
{
	...
	public void initialize(LocalPlayer player, Location current, ApplicableRegionSet set)
	{
		CommandFlagHandlerSupport.dispatchEntryCommandsForApplicableSet(
				...
				set,
				Flags.COMMAND_ON_ENTRY,
				this.lastCommands,
				false);
		this.lastCommands = CommandFlagHandlerSupport.collectCommandSets(set, player, Flags.COMMAND_ON_ENTRY);
	}
			...
			Set<ProtectedRegion> entered, Set<ProtectedRegion> exited, MoveType moveType)
	{
		CommandFlagHandlerSupport.dispatchEntryCommands(
				...
				this.lastCommands,
				false);
		this.lastCommands = CommandFlagHandlerSupport.collectCommandSets(toSet, player, Flags.COMMAND_ON_ENTRY);
		return true;
	}
}
